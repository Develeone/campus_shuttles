...
!function (t, i) {
    ...
    var He = We.extend({
        ...
        options: {
            ...
            var icon = jQuery(this._icon);

            var transformMatrix = jQuery(icon).css("-webkit-transform") ||
                icon.css("-moz-transform")    ||
                icon.css("-ms-transform")     ||
                icon.css("-o-transform")      ||
                icon.css("transform");

            var matrix = transformMatrix.replace(/[^0-9\-.,]/g, '').split(',');

            var transformCode = "rotate("+t+"deg) translate("+matrix[4]+"px,"+matrix[5]+"px)";

            icon.css("transform", transformCode);
            icon.css("transform-origin", (15+parseInt(matrix[4]))+"px "+(15+parseInt(matrix[5]))+"px");
